...
public class SpinnerAdapter extends ArrayAdapter<College> {

    //ArrayAdapter:适配器的实现类，通过“泛型”来指定要适配的数据类型，然后在其构造函数中把要适配的数据传进去

    private int resourceId;

    public SpinnerAdapter(Context context, int textviewResourceId, List<College> collegeList){
    ...
    public View getDropDownView(int position, @Nullable View convertView, @NonNull ViewGroup parent) {
        ...
        if(convertView == null){
            ...
            viewHolder.imageView = view.findViewById(R.id.imageView_item02);
            viewHolder.textView = view.findViewById(R.id.textView_item02);
